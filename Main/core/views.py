    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            Notification.objects.create(
                ...
                message=(
                    'Thank you for joining EventNest! Browse our services '
                    'to plan your perfect event. We offer wedding planning, '
                    'photography, catering, and much more.'
                ),
                link='/services/'
            )
...
def request_service_quote(request, service_id):
    ...
    return render(
        request, 'services/service_detail.html', {'service': service}
    )
...
def my_bookings(request):
    ...
    bookings = (
        Booking.objects.filter(user=request.user).order_by('-created_at')
    )
...
def modify_booking(request, booking_id):
    ...
    if request.method == 'POST':
        ...
        messages.success(
            request,
            f'Booking #{booking.id} has been updated successfully.',
        )
        return redirect('my_bookings')
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        Notification.objects.create(
            ...
            message=(
                f'Your order #{order.id} for ৳{int(total)} has been placed.'
                ' We will process it soon and update you on delivery.'
            ),
            link='/orders/'
        )
...
def api_notifications(request):
    ...
    for n in user_notifications:
        notifications_list.append({
            ...
            'message': (
                n.message[:100] + '...'
                if len(n.message) > 100 else n.message
            ),
...
def download_invoice(request, order_id):
    ...
    try:
        ...
        elements.append(Paragraph(
            ...
            "<br/>EventNest - Event Management Platform",
            footer_style
        ))
